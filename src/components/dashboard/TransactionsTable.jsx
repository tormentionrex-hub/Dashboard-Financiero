...
export default function TransactionsTable() {
  ...
  useGSAP(() => {
    // Animación de entrada y flotación espacial
    ...
    gsap.to(containerRef.current, {
      ...
      yoyo: true,
      ease: "sine.inOut",
      delay: 1.5 // Comienza después de la animación de entrada
    });
  }, { scope: containerRef });
  ...
  return (
    <section ref={containerRef} className="bg-black p-8 md:p-10 rounded-[2.5rem] border border-cyan-500/30 shadow-[0_15px_40px_rgba(6,182,212,0.15)] hover:shadow-[0_20px_50px_rgba(99,102,241,0.25)] transition-shadow duration-700 relative overflow-hidden group">
      ...
      <div className="flex justify-between items-center mb-8 relative z-10">
        <h2 className="text-2xl font-black text-transparent bg-clip-text bg-gradient-to-r from-cyan-300 via-indigo-300 to-indigo-500 tracking-wide uppercase drop-shadow-[0_2px_15px_rgba(6,182,212,0.5)]">Transacciones Clasificadas</h2>
        ...
        <table className="w-full text-left">
          ...
          <tbody>
            {transactions.map((tx, idx) => (
              <tr key={idx} className="table-row border-b border-white/5 hover:bg-gradient-to-r hover:from-white/5 hover:to-transparent transition-all group">

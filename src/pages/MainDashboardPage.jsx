...
import nasaLogo from '../images/logo dashboard.png';
...
const getEmployeeData = (t) => [
  {
    id: 1,
    name: 'Esther Howard',
    position: t('cards.managerUsa'),
    transactions: 3490,
    rise: true,
    tasksCompleted: 3,
    imgId: 0,
  },
  {
    id: 2,
    name: 'Eleanor Pena',
    position: t('cards.managerEurope'),
    transactions: 590,
    rise: false,
    tasksCompleted: 5,
    imgId: 2,
...
const getCountryData = (t) => [
  { name: t('countries.c1'), rise: true,  value: 52340.00, id: 1, flag: 'us' },
  { name: t('countries.c2'), rise: true,  value: 11280.50, id: 2, flag: 'gb' },
  { name: t('countries.c3'), rise: false, value: 8650.75,  id: 3, flag: 'ca' },
  { name: t('countries.c4'), rise: true,  value: 6190.00,  id: 4, flag: 'jp' },
];

const getSegmentationData = (t) => [
  { c1: t('segmentation.c1'), c2: '800', c3: '#363636', color: '#535353' },
  { c1: t('segmentation.c2'), c2: '441', c3: '#818bb1', color: '#595f77' },
  { c1: t('segmentation.c3'), c2: '233', c3: '#2c365d', color: '#232942' },
  { c1: t('segmentation.c4'), c2: '126', c3: '#334ed8', color: '#2c3051' },
];

const getSidebarItems = (t) => [
  [
    { id: '0', title: t('sidebar.home'), path: '/' },
    { id: '1', title: t('sidebar.dashboard'), path: '/main-dashboard' },
    { id: '2', title: t('sidebar.agents'), path: '/agents' },
  ],
  [] // No shortcuts
];

const getGraphData = (t) => {
  const months = t('summary.months');
  return months.map((i) => {
...
export default function MainDashboardPage() {
  ...
  return (
    <div className="admin-dashboard-root flex relative">
      <Sidebar
        onSidebarHide={() => {
...
function Sidebar({ onSidebarHide, showSidebar }) {
  const [selected, setSelected] = useState('1'); // Select dashboard by default
  const { t } = useLanguage();
    ...
    >
      <div className="flex-shrink-0 overflow-hidden p-2">
        <div className="flex items-center h-full sm:justify-center xl:justify-start p-2 sidebar-separator-top">
          <img src={nasaLogo} alt="NASA" className="w-10 h-10 object-contain rounded-full" />
        ...
        <div className="w-full p-3 h-24 sm:h-20 xl:h-24 hidden sm:block flex-shrink-0">
          <div className="bg-sidebar-card-top rounded-xl w-full h-full flex items-center justify-start sm:justify-center xl:justify-start px-3 sm:px-0 xl:px-3">
            ...
            <div className="block sm:hidden xl:block ml-3">
              <div className="text-sm font-bold text-white">{t('sidebar.salesHouse')}</div>
              <div className="text-sm">{t('sidebar.generalItem')}</div>
            </div>
...
function Content({ onSidebarHide }) {
  ...
  const employees = getEmployeeData(t);
    ...
    <div className="flex w-full">
      ...
      <div className=" h-screen flex-grow overflow-x-hidden overflow-auto flex flex-wrap content-start p-2">
        <div className="w-full sm:flex p-2 items-end">
          <div className="sm:flex-grow flex justify-between">
            <div className="">
              <div className="flex items-center">
                <div className="text-3xl font-bold text-white">{t('header.hello')}</div>
              ...
              <div className="flex items-center">
                <Icon
                  ...
                  className="w-3 h-3"
                />
                <div className="ml-2">{t('header.date')}</div>
              </div>
            </div>
            <IconButton
          ...
          <div className="w-full sm:w-80 mt-4 sm:mt-0 relative flex gap-2">
            <div className="relative flex-grow">
              ...
              <input
                type="text"
                name="search"
                className="pl-12 py-2 pr-2 block w-full rounded-lg border-gray-300 bg-card outline-none text-white focus:ring-2 focus:ring-indigo-500"
            ...
            <select 
              ...
              className="bg-[#2d2d2d] text-white rounded-lg px-4 py-2 outline-none cursor-pointer hover:bg-[#3d3d3d] transition border border-[#404040]"
            >
    ...
    <div className="w-full p-2 lg:w-1/3">
      <div className="rounded-lg bg-card flex justify-between p-3 h-32">
        <div className="">
          ...
          <div className="text-sm mt-2">{`${tasksCompleted} ${t('cards.completedTasks')}`}</div>
          <svg
            className="w-44 mt-3"
            height="6"
            viewBox="0 0 200 6"
        ...
        <div className="flex flex-col items-center">
          ...
          <div className="text-sm ">{t('cards.last6Months')}</div>
        </div>
      </div>
    </div>
  );
...
function EmptyState() {
  ...
  return (
    <div className="w-full h-full flex flex-col items-center justify-center p-6 text-center">
      <div className="w-12 h-12 mb-3 rounded-full bg-indigo-500/10 flex items-center justify-center border border-indigo-500/20">
        <Icon path="res-react-dash-premium-star" className="w-6 h-6 text-indigo-400/50" />
      </div>
      <p className="text-gray-400 text-sm mb-4">{t('emptyState.message')}</p>
      <button 
        onClick={() => navigate('/agents')}
        className="px-4 py-2 bg-[#2d2d2d] hover:bg-[#3d3d3d] text-white text-xs rounded-lg transition-colors border border-[#404040] shadow-lg"
      >
        {t('emptyState.button')}
      </button>
    </div>
...
function Graph() {
  const { t } = useLanguage();
  const { isAnalyzed } = useAiAnalysis();
  const graphData = getGraphData(t);

  if (!isAnalyzed) return <EmptyState />;

  const CustomTooltip = ({ active, payload, label }) => {
    if (active && payload && payload.length) {
      return (
        <div className="rounded-xl overflow-hidden tooltip-head shadow-xl border border-[#333]">
          <div className="flex items-center justify-between p-2 bg-[#1d1d1d]">
            <div className="text-white font-bold capitalize">{label}</div>
            <div className="text-xs bg-indigo-500/20 text-indigo-300 px-2 py-0.5 rounded-full ml-3 border border-indigo-500/30">
              {t('aiAnalysis.badge')}
            </div>
          </div>
          <div className="tooltip-body text-center p-3 bg-[#252525]">
            <div className="text-indigo-400 font-bold text-lg">${payload[0].value.toFixed(2)}</div>
            <div className="text-sm text-gray-400 mt-1">{t('summary.revenue')} (Est.)</div>
          </div>
        </div>
      );
    }
  ...
  return (
    <div className="flex p-4 h-full flex-col">
      <div className="">
        <div className="flex items-center">
          <div className="font-bold text-white">{t('summary.title')}</div>
          <div className="text-xs bg-indigo-500/20 text-indigo-300 px-2 py-1 rounded-full border border-indigo-500/30 ml-3 flex items-center gap-1">
            {t('aiAnalysis.badge')}
          </div>
          <div className="flex-grow" />
          <Icon path="res-react-dash-graph-range" className="w-4 h-4" />
          <div className="ml-2">{t('summary.last9Months')}</div>
        </div>
        <div className="font-bold ml-5 mt-2 text-indigo-400 text-sm max-w-sm">
          {t('aiAnalysis.revenueInsight')}
        </div>
      </div>
...
function TopCountries() {
  const { t } = useLanguage();
  const { isAnalyzed } = useAiAnalysis();
  const countries = getCountryData(t);
  const top15 = t('top15Countries');
  const [showModal, setShowModal] = useState(false);
  
  if (!isAnalyzed) return <EmptyState />;
  
  return (
    <div className="flex p-4 flex-col h-full relative">
      <div className="flex justify-between items-center mb-2">
        <div className="text-white font-bold">{t('countries.title')}</div>
        <div className="text-xs bg-indigo-500/20 text-indigo-300 px-2 py-1 rounded-full border border-indigo-500/30 flex items-center gap-1">
          {t('aiAnalysis.badge')}
        </div>
      </div>
      <div className="">{t('countries.favourites')}</div>
      {countries.map(({ name, rise, value, id, flag }) => (
      ...
      <div className="flex justify-center">
        <button onClick={() => setShowModal(true)} className="hover:text-white transition-colors cursor-pointer text-indigo-400 hover:text-indigo-300 font-bold uppercase tracking-widest text-xs">
          {t('countries.checkAll')}
        </button>
      </div>

      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-[#1e1e1e] border border-[#333] rounded-xl w-full max-w-md max-h-[80vh] flex flex-col shadow-2xl">
            <div className="p-4 border-b border-[#333] flex justify-between items-center bg-[#252525] rounded-t-xl">
              <h3 className="text-white font-bold text-lg flex items-center gap-2">
                {top15.title}
              </h3>
              <button onClick={() => setShowModal(false)} className="text-gray-400 hover:text-white text-xl">✕</button>
            </div>
            <div className="p-4 overflow-y-auto flex-grow flex flex-col gap-3 custom-scrollbar">
              {top15.list.map((c, idx) => (
                <div key={idx} className="flex justify-between items-center bg-[#2a2a2a] p-3 rounded-lg border border-[#3a3a3a]">
                  <div className="flex items-center gap-3">
                    <span className="text-gray-400 font-mono text-sm">#{idx + 1}</span>
                    <span className="text-white font-medium">{c.name}</span>
                  </div>
                  <span className="text-indigo-400 font-bold">{c.amount}</span>
                </div>
              ))}
            </div>
            <div className="p-4 border-t border-[#333]">
              <button onClick={() => setShowModal(false)} className="w-full py-2 bg-indigo-600 hover:bg-indigo-500 text-white rounded-lg transition-colors font-semibold">
                {top15.close}
              </button>
            </div>
          </div>
...
function Segmentation() {
  const { t } = useLanguage();
  const { isAnalyzed } = useAiAnalysis();
  const segData = getSegmentationData(t);
  const [showModal, setShowModal] = useState(false);
  const details = t('categoryDetails');
  ...
  return (
    <div className="p-4 h-full relative">
      <div className="flex justify-between items-center mb-2">
        <div className="text-white font-bold">{t('segmentation.title')}</div>
        <div className="text-xs bg-indigo-500/20 text-indigo-300 px-2 py-1 rounded-full border border-indigo-500/30 flex items-center gap-1">
          {t('aiAnalysis.badge')}
        </div>
      </div>
      <div className="mb-4">{t('segmentation.allUsers')}</div>
      {segData.map(({ c1, c2, c3, color }) => (
      ...
      <div 
        ...
        className="flex mt-3 px-3 items-center justify-between bg-details rounded-xl w-36 h-12 cursor-pointer border border-[#333] hover:bg-[#252525] transition-colors"
      >
        <div className="">{t('segmentation.details')}</div>
        <Icon path="res-react-dash-chevron-right" className="w-4 h-4" />
      </div>

      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-[#1e1e1e] border border-[#333] rounded-xl w-full max-w-md shadow-2xl flex flex-col overflow-hidden">
            <div className="p-4 border-b border-[#333] flex justify-between items-center bg-[#252525] rounded-t-xl">
              <h3 className="text-white font-bold text-lg flex items-center gap-2">
                {details.title}
              </h3>
            ...
            <div className="p-5 flex flex-col gap-4 overflow-y-auto max-h-[60vh] custom-scrollbar">
              {['apparel', 'collectibles', 'toys', 'patches'].map((key) => (
                <div key={key} className="bg-[#2a2a2a] p-3 rounded-lg border border-[#3a3a3a]">
                  <h4 className="text-white font-bold mb-1">{details[key].name}</h4>
                  <p className="text-gray-400 text-sm leading-relaxed">{details[key].desc}</p>
                </div>
              ))}
            </div>
            <div className="p-4 border-t border-[#333]">
...
function Satisfication() {
  ...
  return (
    <div className="p-4 h-full relative">
      <div className="flex justify-between items-center mb-2">
        <div className="text-white font-bold">{t('satisfaction.title')}</div>
        <div className="text-xs bg-indigo-500/20 text-indigo-300 px-2 py-1 rounded-full border border-indigo-500/30 flex items-center gap-1">
          {t('aiAnalysis.badge')}
        </div>
      </div>
      <div className="mt-3">{t('satisfaction.fromAll')}</div>
      <div className="flex justify-center">
        <svg
          viewBox="0 0 700 380"
          fill="none"
      ...
      <div className="flex justify-center">
        <div className="flex justify-between mt-2" style={{ width: '300px' }}>
          ...
          >
            ...
            <div className="">{t('satisfaction.basedOnLikes')}</div>
          </div>
    ...
    <div>
      ...
      >
        ...
        <div className="text-white font-bold mt-3">
          {t('addComponent.noComponents')}
        </div>
        <div className="mt-2">{t('addComponent.createFirst')}</div>
        <div className="mt-1">{t('addComponent.clickButton')}</div>
        <div
        ...
        >
          ...
          <div className="ml-2">{t('addComponent.addBtn')}</div>
          <div
            className="ml-2"
            style={{
              background: '#4964ed',
